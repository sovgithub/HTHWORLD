...
var decodeInput = function(tx){
    ...
    tx.ins.forEach(function(input, n){
        var vin = {
            ...
            n : input.index,
            script: hth.script.toASM(input.script),
...
var decodeOutput = function(tx, network){
    ...
    var format = function(out, n, network){
        var vout = {
            ...
            n: n,
            scriptPubKey: {
                asm: hth.script.toASM(out.script),
                hex: out.script.toString('hex'),
                type: hth .script.classifyOutput(out.script),
                addresses: [],
            },
        };
        ...
        case 'pubkeyhash':
        case 'scripthash':
            vout.scriptPubKey.addresses.push(hth.address.fromOutputScript(out.script, network));
            break;
        }
        return vout
    }
...
var TxDecoder = function(rawtx, network){
    this.tx = hth.Transaction.fromHex(rawtx);
